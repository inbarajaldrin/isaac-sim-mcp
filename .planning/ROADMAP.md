...
- [SUPERSEDED] 01-G03-PLAN.md — Gap B (PARITY-04, OGN approach): wire 17 per-frame ROS2PublishRawTransformTree publishers backed by OgnGetPrimWorldPose source nodes. **Superseded by commit `079afd8`** — `parity_publishers.py` bypasses OmniGraph and emits slashed frame_ids via rclpy. The OGN approach was found insufficient: TransformTree OGN derives frame_ids from USD prim leaf names, but USD prims cannot legally contain `/`. Same observable outcome (slash frame_ids on /tf), different mechanism. **Verified superseded 2026-05-08; do NOT execute.**
- [SUPERSEDED] 01-G04-PLAN.md — Gap A (PARITY-03, OGN strategy-pick): ARTICULATION_RESTRUCTURE / SUPPLEMENTARY_PUBLISHER_SUBGRAPH / WRAPPER_REPUBLISHER / HEADER_FRAME_ID_ONLY. **Superseded by commit `079afd8`** — same `parity_publishers.py` publishes 7 alphabetical joints including `gripper/left_finger_joint` with `frame_id=base_link`. OGN ROS2PublishJointState lacks frameId / jointNames / ordering inputs and the gripper joint isn't in articulation discovery from root_joint. Same observable outcome, different mechanism. **Verified superseded 2026-05-08; do NOT execute.**
...
- [ ] 04-3.5-PLAN.md — Runtime payload + round-trip gate before 04-04 ship-gate exercise: PARITY-09 motion deficit hunt (≥95% commanded amplitude), PARITY-10 round-trip re-verify, PARITY-05 wrench root cause via nvidia-suite-docs Articulation lifecycle, gripper/tcp ingress path, truthful state refresh. Bundles 5 CLOSE-NOW items from 04-BACKLOG-SWEEP.md surfaced 2026-05-08; gates 04-04.
- [ ] 04-04-PLAN.md — parity_report.py + first M1 ship-gate exercise (TRIAL-04, TRIAL-05). Gated on Plan 04-3.5 (runtime gate) AND model-zenoh blocker resolution.
