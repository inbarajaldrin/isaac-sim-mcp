...
stopped_at: Phase 2 CLOSED at 6/7 smoke under autonomous M1 mode. Bug 4 root caused as 8 layered bugs (not 1 as the prior verifier diagnosed); all 8 fixed across commits a3b207b + c9283ae + 6ef405a. Phase 1 smoke 13/13 maintained under ROS_DOMAIN_ID=7. Step 3 PARITY-09 single-shot ~50% motion is documented carry-forward (real aic_controller pre-smooths trajectories, per-cmd deltas << 0.05 rad). Steps 4 (PARITY-10 EE motion via Lula IK) + Step 5 (PARITY-11 controller_state nonzero tcp_pose + tcp_error) BOTH PASSING fully. Autonomous mode + hooks + CLAUDE.md bootstrap block all in place — next session can resume cold. Next: /gsd-discuss-phase 3 (cable physics + object TF).
last_updated: "2026-05-05T10:30:00.000Z"
last_activity: 2026-05-05 -- Bug 4 deep debug session: 5 fixes for the 5 layered root causes (joint_names= API quirk, _physics_view init timing, OGN race, PD target persistence, error-flag separation) + 3 follow-on fixes (SingleArticulation FK, ROS_DOMAIN isolation, one-shot apply with cmd-change guard). Phase 2 smoke 3/7 → 5/7 → 6/7. controller_loop.py final: ~1330 LOC. Autonomous M1 mode infrastructure landed (.claude/settings.json hooks + scripts + CLAUDE.md bootstrap block + .planning/.autonomous_m1_active marker). Cache snapshot taken (DerivedDataCache.bak.1777975428).
progress:
  total_phases: 4
  completed_phases: 2
  total_plans: 15
