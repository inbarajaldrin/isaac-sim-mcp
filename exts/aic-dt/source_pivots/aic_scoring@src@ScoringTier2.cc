...
bool ScoringTier2::StartRecording(const std::string &_filename,
  ...
  for (const auto &topic : this->topics) {
    ...
    // Plan 04-03 host-build pivot: humble create_generic_subscription takes
    // a 1-arg callback (SerializedMessage), not 2-arg (with MessageInfo).
    // bagWriter.write also has a humble-compatible 3-arg overload.
    auto sub = this->node->create_generic_subscription(
        ...
        [this, topic](std::shared_ptr<rclcpp::SerializedMessage> msg) {
          ...
          if (this->state == State::Recording) {
            // humble rosbag2_cpp::Writer::write 3-arg overload:
            // (SerializedMessage, topic_name, time_stamp)
