...
#include <cstring>
#include <deque>
// Plan 04-03 host-build pivot: g++-11 lacks C++20 <format>; use libfmt.
  ...
  for (size_t camera_idx = 0; camera_idx < kNumCameras; camera_idx++) {
    camera_info_subs_.push_back(
        this->create_subscription<sensor_msgs::msg::CameraInfo>(
            fmt::format("/{}_camera/camera_info", kCameraNames[camera_idx]), 5,
    ...
    image_subs_.push_back(this->create_subscription<sensor_msgs::msg::Image>(
        fmt::format("/{}_camera/image", kCameraNames[camera_idx]), 5,
